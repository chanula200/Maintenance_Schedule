<%@ Page Title="" Language="C#" MasterPageFile="~/Site1.Master" AutoEventWireup="true" CodeBehind="pwdb.aspx.cs" Inherits="Routine_Maintenance.pwdb" %>
        ...
        <asp:GridView ID="GridView2" runat="server" AutoGenerateColumns="False" BackColor="White" BorderColor="#3366CC" BorderStyle="None" BorderWidth="1px" CellPadding="4" DataSourceID="SqlDataSource2" OnRowCommand="GridView2_RowCommand">
            <Columns>
                ...
                <asp:TemplateField HeaderText="Closed" SortExpression="Closed">
                    <EditItemTemplate>
                        <asp:Label ID="Label2" runat="server" Text='<%# Eval("Closed") %>'></asp:Label>
                    </EditItemTemplate>
                    <ItemTemplate>
...
SELECT ntw_eng, 
 ...
 SUM(CASE WHEN status = 'close' THEN 1 ELSE 0 END) AS Closed FROM schedule left join lea2 on lea2.lea=schedule.lea where Platform='MSAN' and month(Start_Date)=month(@start)  and year(Start_Date)=year(@start)  group by ntw_eng
">
            <SelectParameters>
